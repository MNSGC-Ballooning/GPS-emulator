...
void dataSort(String pass) {
  ...
  for (int i = 0; i < pass.length(); i++) { //get seperating comma chars and set semi & col to index
  ...
  if (String(dataPass).length() != pass.length()) { //if data is not same length, place null at end of GPSchars to prevent memory from last loop carrying over
    dataPass[String(dataPass).length() + 1] = '\0';
  }
